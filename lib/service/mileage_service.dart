...
class MileageService {
  ...
  final mileageUrl = 'https://project-568166903627460027-default-rtdb.firebaseio.com/mileage.json';
  final withdrawUrl = 'https://project-568166903627460027-default-rtdb.firebaseio.com/withdraw.json';

  Future<void> saveMileage(Mileage mileage) async {
    try {
      // 유저 마일리지 업데이트
      final value = await firestore.collection('user').document(mileage.call).get();

      int updatedMileage = value.map['mileage'] + mileage.amount;
      mileage.sumMileage = updatedMileage;
      await firestore.collection('user').document(mileage.call).update({'mileage': updatedMileage});

      await firestore.collection('user').document(mileage.call).collection('mileage').document(mileage.orderNumber + mileage.type).set(mileage.toJson());

      final a = await http.post(Uri.parse(mileageUrl), body: jsonEncode(mileage));

      final mileageNum = jsonDecode(a.body)['name'];
      await firestore.collection('mileage').document(mileage.date.substring(0, 7)).collection(mileage.date.substring(8, 10)).document(mileageNum).set({'mileage': mileageNum});
  ...
  Future<void> updateWithdraw(Withdraw withdraw, String status) async {
    try {
      final userData = await firestore.collection('user').document(withdraw.userCall).get();
      // 유저 마일리지 기록 저장 -> 출금
      ...
      if (status == "출금완료") {
        ...
        await firestore.collection('user').document(withdraw.userCall).update({'mileage': updatedMileage});
      }
      final withdrawMileage = status == "출금완료" ? userData.map['mileage'] - withdraw.amount - 500 : userData.map['mileage'];
      ...
      // 관리자용 출금 기록 저장
      await firestore.collection('withdraw').document(withdraw.createdAt + withdraw.userCall).update({'status': status, 'sumMileage': withdrawMileage});
    } catch (e) {
  ...
  Future<List<Withdraw>> getWithdraw() async {
    ...
    try {
      ...
      await Future.wait(withdrawList.map((e) async {
        await firestore.collection('withdraw').document(e.createdAt + e.userCall).get().then((value) {
  ...
  Future<List<Mileage>> getMileageRecordUser(String userCall) async {
    ...
    try {
      await firestore.collection('user').document(userCall).collection('mileage').get().then((value) => value.forEach((element) {
  ...
  Future<List<Withdraw>> getWithdrawRecordUser(String userCall) async {
    List<Withdraw> withdrawList = [];
    try {
      await firestore.collection('user').document(userCall).collection('withdraw').get().then((value) => value.forEach((element) {
  ...
  Future<List<Mileage>> getMileageForDate(String? date) async {
    ...
    List<Mileage> mileageList = [];
    try {
      await firestore.collection('mileage').document('202302').collection('25').get().then((value) {
